...
class Ledge(Level):
    ...
    def build(self):
        ...
        back_wall_end = build_array(Ground, self.game, pos, backwall_dim, xreverse=True, yreverse=True)
        
        # lower ledge
        pos = get_end_pos(Ground, bottom_left_pos, lower_ledge_pos, yreverse=True)
        build_row(Platform, self.game, pos, 2, world="two")

        # back ledge
        pos = get_end_pos(Ground, back_wall_end, (2, 1), xreverse=True)
        back_ledge_end = build_row(Ground, self.game, pos, back_ledge_width, reverse=True)
        
        # under ledge charge and enemy
        pos = get_end_pos(Ground, bottom_right_pos, (backwall_dim[0] + 2, 2), xreverse=True, yreverse=True)
        build_row(WarpCharge, self.game, pos, 1)
        build_row(DumbEnemy, self.game, pos, 1, world="one")

        # on backwall charge and enemy
        pos = get_end_pos(Ground, bottom_right_pos, (3, backwall_dim[1] + 2), xreverse=True, yreverse=True)
        build_row(WarpCharge, self.game, pos, 1)
        build_row(Chaser, self.game, pos, 1, world="two")
        
        # middle ledge
        pos = get_end_pos(Ground, back_ledge_end, (4, 3.7), yreverse=True, xreverse=True)
        middle_ledge_end = build_row(Platform, self.game, pos, 1, reverse=True, world="one")

        # mid air charge
        pos = get_end_pos(Ground, middle_ledge_end, (4, 4), xreverse=True, yreverse=True)
        build_row(WarpCharge, self.game, pos, 1)
        
        # victory ledge
        pos = get_end_pos(Ground, middle_ledge_end, (7, 1.7), xreverse=True)
        flag_root = build_row(Ground, self.game, pos, 5, reverse=True, world="two")
        
        # goal
        pos = get_end_pos(Ground, flag_root, (1, 2), yreverse=True)
